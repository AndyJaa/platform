                        ...
                        <div v-if="$store.state.menu.routes.length > 1" class="nav">
                            <template v-for="(item, index) in $store.state.menu.routes">
                                <div v-if="item.children && item.children.length !== 0" :key="index" :class="{
                                    'item': true,
                                    'active-header-item': index == $store.state.menu.headerActived
...
header {
    ...
    .nav {
        ...
        .item {
            ...
            &.active-header-item {
                border-bottom: 0.04rem solid $g_main_sidebar_menu_active_text_color;
                border-radius: 0;
                color: $g_main_sidebar_menu_active_text_color;
            }
        }
    }
    ::v-deep .user {
...
.wrapper {
    ...
    .sub-sidebar-container {
        ...
        top: 0;
        bottom: 0;
        box-shadow: 0.11rem 0 0.11rem -0.11rem darken($g-sub-sidebar-bg, 20);
        &.is-collapse {
            width: 1rem;
            .sidebar-logo {
                &:not(.sidebar-logo-bg) {
        ...
        .sidebar-logo {
            ...
            &.shadow {
                box-shadow: 0 0.11rem 0.11rem -0.11rem darken($g-sub-sidebar-bg, 20);
            }
        }
        .el-menu {
            border-right: 0;
    ...
    .main-container {
        ...
        box-shadow: 0.01rem 0 0 0 darken($g-main-bg, 10);
        transition: 0.3s;
        .main {
            height: 100%;
            flex: auto;
            position: relative;
            padding: $g-topbar-height + 0.11rem 0 0;
            overflow: hidden;
        }
    }
}
