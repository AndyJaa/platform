<template>
    <div id="sideItem_index" v-if="item.meta.sidebar !== false">
        <router-link v-if="!hasChildren" v-slot="{ href, navigate, isActive, isExactActive }"
                     :to="resolvePath(item.path)">
            <a :href="isExternal(resolvePath(item.path)) ? resolvePath(item.path) : href"
               :class="[isActive && 'router-link-active', isExactActive && 'router-link-exact-']"
               :target="isExternal(resolvePath(item.path)) ? '_blank' : '_self'" @click="navigate">
    ...
    ::v-deep .el-submenu {
        .el-menu--inline {
            ...
            .el-menu-item {
                ...
                &:hover {
                    background-color: lighten($g_sub_sidebar_bg_hover, 10) !important;
                }
            }
        }
    }
